...
def search_patients():
    ...
    pagination = patient_query.paginate(
        page=page,
        per_page=10,
        error_out=False
    )
...
def register_patient():
    ...
    if request.method == 'POST':
        ...
        same_year_patients = Patient.query.filter(
        ...
        log_action(
            ...
            details=(
                ...
                f"with MRN: {patient_number}."
            )
        )

        flash(
            f"Patient {new_patient.full_name} successfully registered. "
            f"MRN: {patient_number}",
            "success"
        )
...
def view_patient_profile(patient_id):
    ...
    if (
        ...
        abort(
            403,
            description=(
                "This patient record is currently inactive."
            )
        )
    ...
    has_clinical_access = role_name in [
        'Doctor',
        'Administrator',
        'Hospital System Administrator'
    ]
...
@main_bp.route('/patients/<int:patient_id>/schemes')
@login_required
@role_required([
    'Receptionist',
    'Administrator',
...
def check_patient_schemes(patient_id):
    ...
    if not patient.is_active:
        ...
        abort(
            403,
            description=(
                "Cannot check schemes for "
                "an inactive patient record."
            )
        )

    # Annual income is deliberately optional
    # during normal patient registration/editing.
    #
    # It becomes mandatory only when the user
    # attempts to check government schemes.

    if patient.annual_income is None:

        flash(
            "Annual income is required to check "
            "government schemes. Please update "
            "the patient's profile first.",
            "warning"
        )
    ...
    # Scheme eligibility page will be connected
    # after the scheme module is implemented.
    return render_template(
        'schemes/check.html',
        patient=patient
    )
...
def edit_patient(patient_id):
    ...
    if not patient.is_active:
        ...
        abort(
            403,
            description=(
                "Cannot edit an inactive patient record."
            )
        )
    ...
    if request.method == 'POST':
        ...
        annual_income_str = request.form.get(
            'annual_income',
            ''
        ).strip()
        ...
        # Annual income is optional here.
        # Empty value means the patient simply
        # has no income information recorded yet.
        ...
        patient.annual_income = annual_income
        ...
        log_action(
            ...
            details=(
                f"Updated demographics details for "
        ...
        flash(
            "Patient demographic details updated successfully.",
            "success"
        )
...
@main_bp.route(
    '/patients/<int:patient_id>/toggle-active',
    methods=['POST']
)
@login_required
@role_required([
...
@main_bp.route(
    '/patients/documents/download/<int:doc_id>'
)
@login_required
def download_document(doc_id):
    ...
    if role_name not in [
        ...
        abort(
            403,
            description="Unauthorized document access."
        )
...
def manage_appointments():
    ...
    if request.method == 'POST':
        ...
        patient_id_str = request.form.get(
            'patient_id',
            ''
        )

        doctor_id_str = request.form.get(
            'doctor_id',
            ''
        )

        appt_date_str = request.form.get(
            'appointment_date',
            ''
        )
        ...
        if (
            not patient_id_str
            or not doctor_id_str
            or not appt_date_str
        ):
        ...
        log_action(
            ...
            details=(
                f"Scheduled appointment for patient "
                f"{patient.full_name} "
                f"({patient.patient_number}) "
                f"with {doctor.full_name} on "
                f"{appt_date.strftime('%Y-%m-%d %H:%M')}."
            )
        )
    ...
    if q:
        ...
        appt_query = (
            ...
            .join(Patient)
            .filter(
                Patient.first_name.like(
                    f"%{q}%"
                ) |
                Patient.last_name.like(
                    f"%{q}%"
                ) |
                Patient.patient_number.like(
                    f"%{q}%"
...
@main_bp.route(
    '/appointments/<int:appt_id>/status',
    methods=['POST']
)
@login_required
@role_required([
...
def update_appointment_status(appt_id):
    ...
    if status in [
        ...
        log_action(
            ...
            details=(
                f"Updated appointment ID "
                f"{appt.id} status for patient "
                f"{appt.patient.full_name} from "
    ...
    return redirect(
        request.referrer
        or url_for('main.manage_appointments')
    )